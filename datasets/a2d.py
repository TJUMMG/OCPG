...
class A2DSentencesDataset(Dataset):
    ...
    def __init__(self, image_folder: Path, ann_file: Path, transforms, return_masks: bool, 
                 num_frames: int, max_skip: int, subset, supervision):
        super(A2DSentencesDataset, self).__init__()
        dataset_path = str(image_folder)
        self.supervision = supervision
        self.mask_annotations_dir = os.path.join(dataset_path, 'text_annotations/a2d_annotation_with_instances_weakly')
        ...
        while not instance_check:
            ...
            if self.subset == 'train':
                ...
                for i in range(start_idx, end_idx):
                    i = min(max(i, 0), vid_len-1)  # pad out of range indices with edge frames
            ...
            weak_box[0::2] = weak_box[0::2] * w
            weak_box[1::2] = weak_box[1::2] * h
            weak_masks.append(weak_mask)
            weights.append(heatmaps[instance_idx])
            ...
            target = {
                ...
                'masks': masks,                          # [1, H, W]
                "weights": weights,               # [1, H, W]
                "weak_masks": weak_masks,             # [1, H, W]
...
def build(image_set, args):
    ...
    dataset = A2DSentencesDataset(img_folder, ann_file, transforms=make_coco_transforms(image_set, max_size=args.max_size), 
                                return_masks=args.masks, num_frames=args.num_frames, max_skip=args.max_skip, subset=image_set, supervision=args.supervision)
    return dataset