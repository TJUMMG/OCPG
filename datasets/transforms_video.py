...
class Check(object):
    ...
    def __call__(self, img, target):
        ...
        if "masks" in target:
            fields.append("masks")
        if "weak_masks" in target:
            fields.append("weak_masks")
...
def crop(clip, target, region):
    ...
    if "weak_masks" in target:
        # FIXME should we update the area here if there are no boxes?
        target["weak_masks"] = target["weak_masks"][:, i : i + h, j : j + w]
        fields.append("weak_masks")
    if "weights" in target:
        # FIXME should we update the area here if there are no boxes?
    ...
    if "points" in target:
        ...
        cropped_points = torch.min(cropped_points, max_size)
    ...
    if "boxes" in target:
        ...
        #     if (mask > 0).any():
        #         y1, y2, x1, x2 = bounding_box(mask)
        #         bboxes.append(torch.tensor([x1, y1, x2, y2]).to(torch.float))
        #         valid.append(1)
        #     else:
        #         bboxes.append(torch.tensor([0, 0, 0, 0]).to(torch.float))
        #         valid.append(0)
...
def hflip(clip, target):
    ...
    if "weak_masks" in target:
        target["weak_masks"] = target["weak_masks"].flip(-1)
...
def resize(clip, target, size, max_size=None):
    ...
    if "masks" in target:
        if target["masks"].shape[0] > 0:
            target["masks"] = (
                interpolate(target["masks"][:, None].float(), size, mode="nearest")[
                    :, 0
                ]
                > 0.5
            )
    ...
    if "weak_masks" in target:
        if target["weak_masks"].shape[0] > 0:
            target["weak_masks"] = interpolate(
                target["weak_masks"][:, None].float(), size, mode="bilinear"
            )[:, 0]
        else:
            target["weak_masks"] = torch.zeros((target["weak_masks"].shape[0], h, w))
    if "weights" in target:
        if target["weights"].shape[0] > 0:
            target["weights"] = interpolate(
                target["weights"][:, None].float(), size, mode="bilinear"
            )[:, 0]
        else:
            target["weights"] = torch.zeros((target["weights"].shape[0], h, w))
...
class Expand(object):
    ...
    def __call__(self, clip, target):
        ...
        for i in range(len(clip)):
            ...
            expand_mask = torch.zeros(
                (int(height * ratio), int(width * ratio)), dtype=torch.uint8
            )
            expand_mask[
                int(top) : int(top + height), int(left) : int(left + width)
            ] = target["masks"][i]
