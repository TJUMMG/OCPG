...
class OCPG(nn.Module):
    ...
    def __init__(self, args, backbone, transformer, num_classes, num_queries, num_feature_levels,
                    ...
                    freeze_text_encoder=False, freeze_video_encoder=False, rel_coord=True, matcher=None):
        ...
        self.ls_feat_viz = nn.Conv2d(hidden_dim, 8, 3, 1, 1)
        self.ls_text_proj = nn.Linear(hidden_dim, 8)
    ...
    def init_aux_head(self):
        ...
        nn.init.xavier_uniform_(self.ls_feat_viz.weight, gain=1)
        nn.init.constant_(self.ls_feat_viz.bias, 0)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        if 'valid_indices' in targets[0]:
            ...
            samples.tensors = samples.tensors.index_select(0, valid_indices)
            t = 1
        ...
        mask_features = rearrange(memory_fusion, '(b t) c h w -> b t c h w', b=b, t=t)  # [b, t, 256, h, w]
        ...
        if self.training:
            ...
            if self.aux_loss:
                ##############
                ls_feat_viz = self.ls_feat_viz(memory_fusion)
                ls_feat_viz = F.interpolate(ls_feat_viz, scale_factor=4, mode='bilinear', align_corners=True)
                ls_feat_viz = rearrange(ls_feat_viz, '(b t) c h w -> b t c h w', b=b, t=t)  # [b, t, 256, 4h, 4w]

                text_sentence_features_proj = self.ls_text_proj(text_sentence_features)
                text_sentence_features_expand = text_sentence_features_proj[:, None, :, None, None]
                sim_cross = (ls_feat_viz * text_sentence_features_expand).sum(dim=2) / ((F.normalize(ls_feat_viz, dim=2) * F.normalize(text_sentence_features_expand, dim=2)).sum(dim=2) + 1e-5)
                img_ori = F.interpolate(samples.tensors, ls_feat_viz.shape[-2:], mode='bilinear', align_corners=True)
                img_ori = rearrange(img_ori, '(b t) c h w -> b t c h w', b=b, t=t)  # [b, t, 3, 4h, 4w]
                
                ls_features = torch.cat([img_ori, ls_feat_viz, sim_cross.unsqueeze(2)], dim=2) # [b, t, 3+8+1, 4h, 4w]
                ls_features = repeat(ls_features, 'b t c h w -> b (t q) c h w', q=self.num_queries)
                ls_features = rearrange(ls_features, 'b (t q) c h w -> b q t c h w', t=t)[select_idxs]  # b t c h w
                ### 
                ...
                pred_masks = [F.interpolate(x, scale_factor=4) for x in pred_masks]
                ...
                out['aux_outputs'] = self._set_aux_loss_comprehensive(outputs_class, outputs_coord, outputs_seg_masks, outputs_seg_masks_formatcher, ls_features, img_ori)
            ...
            pred_masks = F.interpolate(pred_masks, scale_factor=4)
            ...
            for idx_ in range(self.num_queries):
                ...
                pred_masks_ = F.interpolate(pred_masks_, scale_factor=4)
    ...
    @torch.jit.unused
    def _set_aux_loss_comprehensive(self, outputs_class, outputs_coord, outputs_seg_masks, outputs_seg_masks_low, ls_features, img_ori):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c, "pred_masks_low": d, "ls_features": e, 'frames': f}
                for a, b, c, d, e, f in zip(outputs_class[:-1], outputs_coord[:-1], outputs_seg_masks[:-1], outputs_seg_masks_low[:-1], [ls_features for _ in range(len(outputs_class[:-1]))], [img_ori for _ in range(len(outputs_class[:-1]))])]
...
def build(args):
    ...
    model = OCPG(
        ...
        freeze_video_encoder=args.freeze_video_encoder,
    ...
    if args.masks:
        ...
        weight_dict['loss_proj'] = args.proj_loss_coef
        weight_dict['loss_lst'] = args.lst_loss_coef
        weight_dict['loss_mask_low'] = args.mask_loss_coef
        weight_dict['loss_dice_low'] = args.dice_loss_coef
        weight_dict['loss_proj_low'] = args.proj_loss_coef
        weight_dict['loss_lst_low'] = args.lst_loss_coef
